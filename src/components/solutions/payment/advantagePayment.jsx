...
export default function AdvantagePayment() {
  return (
    <div
      className="
    h-fit w-full flex flex-col relative py-32 justify-between items-center "
    >
      <div
        className="
      font-pre-bold text-3xl sm:text-[44px] pb-2 flex text-center justify-center gradientText"
      >
      ...
      >
        <img
          ...
          alt={"YourDPay"}
          className="
          object-contain sm:w-[550px] rounded-3xl "
        />
        <div
          className="
        flex flex-col gap-8 mt-16 h-full"
        >
          {advantageFeatures.map((feature, index) => (
            <div
              className="
            flex hover:scale-105 duration-200 transform transition-transform gap-6 bg-white rounded-[20px] p-3 sm:p-7"
            >
              <img
                src={feature.icon}
                alt={feature.title}
                className="
                object-contain w-[28px] h-[28px] sm:w-[32px] sm:h-[32px] md:w-[40px] md:h-[40px]"
              />
              <div key={index} className={`flex flex-col gap-5 `}>
                <div
                  className="
                font-pre-bold text-lg md:text-xl leading-5 text-left"
                >
                  {feature.title}
                </div>
                <p
                  className="
                font-pre-regular font-bold text-sm h-fit sm:text-[16px] text-gray300"
                >
                  {feature.text}
                </p>
              </div>
